jQuery(window).on( 'load', function() {
    ...
    var $window = $('window'),
        ...
        $header_nav = $header.find('.header-nav'),
        ...
        is_nav_anim_2 = $header.hasClass('header-style-4-nav-anim-2'),
    ...
    /*---------------------------------------------------------------------------------*/
    /*  Vertical Menu (Header Style 4)
    /*---------------------------------------------------------------------------------*/
    
    // Navigation
    if ( is_header_style_4 ) {

        if ( is_nav_anim_2 ) {

            $header_nav.find('ul').css('height', function() {
                return $(this).height();
            })

            $header.find('.nav-item-has-submenu').children('a').on('click', function(e) {
                e.preventDefault()

                var $parent_list = $(this).closest('ul');

                TweenMax.to( $parent_list.children('li').children('a'), 0.3, { css: { z: '+=300' }, ease: Power1.easeInOut, delay: 0 });
                TweenMax.to( $parent_list.children('li').children('a'), 0.3, { css: { opacity: 0, display: 'none' }, ease: Power1.easeInOut, delay: 0 });

                TweenMax.to( $(this).siblings('ul'), 0, { css: { display: 'block' }, ease: Power1.easeInOut, delay: 0 });
                TweenMax.to( $(this).siblings('ul').children('li').children('a'), 0, { css: { z: '-300' }, ease: Power1.easeInOut, delay: 0 });
                TweenMax.to( $(this).siblings('ul').children('li').children('a'), 0.3, { css: { z: '0', opacity: 1, display: 'block' }, ease: Power1.easeInOut, delay: 0 });
            });

            $header.find('.nav-item-back-btn').children('a').on('click', function(e) {
                e.preventDefault()

                var $parent_list = $(this).closest('ul').parent().closest('ul');

                TweenMax.to( $(this).closest('ul').children('li').children('a'), 0.3, { css: { z: '-300', opacity: 0, display: 'none' }, ease: Power1.easeInOut, delay: 0 });

                TweenMax.to( $parent_list.children('li').children('a'), 0, { css: { z: '+300', opacity: 0, display: 'block' }, ease: Power1.easeInOut, delay: 0 });
                TweenMax.to( $parent_list.children('li').children('a'), 0.3, { css: { z: '0', opacity: 1 }, ease: Power1.easeInOut, delay: 0 });
                TweenMax.to( $(this).closest('ul'), 0, { css: { display: 'none' }, ease: Power1.easeInOut, delay: 0.3 });
            ...
            $header.find('.nav-item-has-submenu').children('a').on('click', function(e) {
                ...
                TweenMax.to( $submenu, 0.3, { css: { opacity: 1 }, ease: Power2.easeOut, delay: 0 });
                TweenMax.to( $header_nav_list, 0.3, { css: { x: '-=269' }, ease: Power2.easeOut, delay: 0 });
            });
            $header.find('.nav-item-back-btn').children('a').on('click', function(e) {
                e.preventDefault()
                var $submenu = $(this).closest('.nav-submenu');
                TweenMax.to( $submenu, 0.3, { css: { opacity: 0 }, ease: Power2.easeOut, delay: 0 });
                TweenMax.to( $header_nav_list, 0.3, { css: { x: '+=269' }, ease: Power2.easeOut, delay: 0 });
            });
        }
    ...
    function header_style_4_responsive_handle() {
        var header_style_class = $header.attr('data-header-style');
        if ( $(window).width() < $header.attr('data-main-nav-threshold-width') ) {
            $header.removeClass(header_style_class);
        } else {
            $header.addClass(header_style_class);
        }
    }
    // Responsive Handle
    if ( is_header_style_4 ) {
        var _header_style_4_responsive_handle = _.throttle(header_style_4_responsive_handle, 100);
        $(window).resize(_header_style_4_responsive_handle);
        header_style_4_responsive_handle();
    }
    ...
    if ( ~sticky_offset.indexOf("%") ) {  // If offset is defined as percentage
        sticky_offset = sticky_offset.replace("%", "");
        sticky_offset = ( parseInt(sticky_offset, 10) / 100 ) * $(document).height();
    }
